This repository contains the code for iSuite CodeX, an automated AI software engineer. It has a Python backend
...
## Running iSuite CodeX with iSuite CodeX:
...
The `enterprise/` directory contains additional functionality that extends the open-source iSuite CodeX codebase. This includes:
...
1. First, build the main iSuite CodeX project: `make build`
...
- The enterprise server extends the iSuite CodeX server through dynamic imports
- Database changes require careful migration planning in `enterprise/migrations/`
- Always test changes in both iSuite CodeX and enterprise contexts
...
- This ensures code works in both iSuite CodeX and enterprise contexts
...
- For frontend issues, ensure the main iSuite CodeX frontend is built: `make build`
...
Microagents are specialized prompts that enhance iSuite CodeX with domain-specific knowledge and task-specific workflows. They are Markdown files that can include frontmatter for configuration.
...
- To add a new user setting to iSuite CodeX, follow these steps:
...
There are two main patterns for saving settings in the iSuite CodeX frontend:
...
To add a new LLM model to iSuite CodeX, you need to update multiple files across both frontend and backend:
...
1. **Frontend Model Arrays** (`frontend/src/utils/verified-models.ts`):
   ...
   - Add to provider-specific arrays based on the model's provider:
     ...
     - `VERIFIED_OPENHANDS_MODELS` for models available through iSuite CodeX provider
...
3. **Backend Model List** (`openhands/utils/llm.py`):
   - **CRITICAL**: Add the model to the `openhands_models` list (lines 57-66) if using iSuite CodeX provider
...
- **VERIFIED_OPENHANDS_MODELS**: Models available through iSuite CodeX managed provider
